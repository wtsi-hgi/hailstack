...
* Users select from named preset bundles in the compatibility matrix (e.g. "hail-0.2.137"), not individual component versions. Each bundle defines the full set: Spark, Hadoop, Hail, Java/JDK, Python, Scala, Gnomad. The matrix is repo-locked; changes require PRs to the checked-in TOML file.
...
* The compatibility matrix TOML is structured hierarchically by Hail version with nested Spark/Hadoop/Java/Python/Scala/Gnomad combos. A `default` marker identifies the latest recommended bundle.
...
* Packer images are named after their bundle (e.g. `hailstack-hail-0.2.137`). The `create` subcommand auto-discovers the image in OpenStack Glance by name based on the selected bundle. The `build-image` subcommand uploads images with this naming convention.
